...
export async function saveItem(userId, data) {
  console.log('Saving item for user:', userId, 'Type:', data.type);
  try {
    const aiData = await generateTagsAndSummary(data.content || '', data.type || 'note', data.title || '', data.url || '');
    console.log('AI Data generated:', aiData);
...
export async function deleteItem(itemId) {
  ...
  try {
    ...
    console.warn('Error finding relations to delete:', err);
    // Continue anyway, we want to at least delete the item
  }
